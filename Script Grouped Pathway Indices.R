...
aov <- aov(CSF_NE_DHPG_MHPG ~ ID, data = combineddata) %>%
  tukey_hsd(conf.level = .95)
n1 <-ggboxplot(combineddata , x = "ID", y = "CSF_NE_DHPG_MHPG", outlier.shape = NA) +
...
# anova
aov2 <- aov(CSF_DA_DOPAC_HVA ~ ID, data = combineddata) %>%
  tukey_hsd(conf.level= 0.95)
d1 <- ggboxplot(combineddata , x = "ID", y = "CSF_DA_DOPAC_HVA",  outlier.shape = NA) +
  geom_jitter(aes(fill = ID),width = .2, height = 0, size = 4, shape = 21, stroke = 1 ) +
  stat_boxplot(geom ='errorbar', width = 0.4) +
  guides(y = guide_axis(minor.ticks = TRUE))+
  scale_fill_manual(values  = c("grey", "blue", "magenta",  "red"))+
